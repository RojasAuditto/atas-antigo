...
import { importLegacyMovements } from "../../../../../src/services/distribution-service";

const ROUTE = "/api/v1/movements/import";
...
export async function POST(request: Request): Promise<Response> {
  return handleDistributionRequest(request, ROUTE, ALLOWED_METHODS, async ({ requestId }) =>
    successResponse(
      await importLegacyMovements(movementRepository, await readJsonBody(request)),
      requestId,
    ),
  );
}
